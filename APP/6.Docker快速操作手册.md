...
Docker Hub 上有 8,453,009 的应用，开发者可以从中选取一个进行快速扩展开发。Docker 跟踪管理变更和依赖关系，让系统管理员能更容易理解开发人员是如何让应用运转起来的。而开发者可以通过 Docker Hub 的共有/私有仓库，构建他们的自动化编译，与其他合作者共享成果。—— [Docker Hub](https://hub.docker.com/)
...
如果你需要映射宿主机 (host) 的目录到 Docker 容器内，可使用 `docker run -v $HOSTDIR:$DOCKERDIR`。详见 [卷标(Volumes)](https://m24y.com/index.php/2021/11/11/docker-guide/#卷标volumes) 一节。
...
如果你想将容器的端口 (ports) 暴露至宿主机，请见 [暴露端口](https://m24y.com/index.php/2021/11/11/docker-guide/#暴露端口exposing-ports) 一节。
...
虽然你可以用 `docker rmi` 命令来删除指定的镜像，不过有个名为 [docker-gc](https://github.com/spotify/docker-gc) 的工具，它可以以一种安全的方式，清理掉那些不再被任何容器使用的镜像。Docker 1.13 起，使用 `docker image prune` 亦可删除未使用的镜像。参见 [清理](https://m24y.com/index.php/2021/11/11/docker-guide/#清理)。
...
### 5.6 加载已保存的镜像 与 导入已导出为镜像的容器 的不同
...
记得，[文件也可以被挂载为卷标](https://m24y.com/index.php/2021/11/11/docker-guide/#将文件挂载为卷标)。

## 10 暴露端口(Exposing ports)
